  ...
  SelectBox.prototype.showMenu = function () {
    ...
    var pos = control.offset(),
      ...
      posTop =
        ...
        (settings.keepInViewport == null ? true : settings.keepInViewport),
      width =
    ...
    options
      ...
      .addClass(
        'selectBox-options selectBox-options-' + (posTop ? 'top' : 'bottom'),
      )
    ...
    switch (event.keyCode) {
      ...
      default:
        ...
        options.find('A').each(function () {
          if (
            $(this).text().substr(0, self.typeSearch.length).toLowerCase() ===
            self.typeSearch.toLowerCase()
          ) {
            self.addHover($(this).parent())
            self.selectOption($(this).parent(), event)
