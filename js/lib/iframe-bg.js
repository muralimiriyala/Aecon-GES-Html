;(() => {
  function buttonOn(buttonObj) {
    ...
    buttonObj.element.firstChild.classList.remove(
      buttonObj.stateChildClassNames[0],
    )
    buttonObj.element.firstChild.classList.add(
      buttonObj.stateChildClassNames[1],
    )
  ...
  function isUserAgentMobile(str) {
    return (
      /\b(BlackBerry|webOS|iPhone|IEMobile)\b/i.test(str) ||
  ...
  var SuperVideoBackground = class {
    constructor(elem, params, id, uid, type, factoryInstance) {
      ...
      this.params = this.parseProperties(params, DEFAULTS, this.element, [
        'data-ytbg-',
        'data-vbg-',
      ])
    ...
    dispatchEvent(name) {
      this.element.dispatchEvent(
        new CustomEvent(name, { bubbles: true, detail: this }),
      )
    }
    shouldPlay() {
    ...
    mobileLowBatteryAutoplayHack() {
      ...
      document.addEventListener('touchstart', forceAutoplay.bind(this), {
        once: true,
      })
    }
  ...
  var VimeoBackground = class extends SuperVideoBackground {
    ...
    injectScript() {
      ...
      if (
        ...
        typeof window.onVimeoIframeAPIReady === 'function'
      )
    ...
    generateSrcURL(id) {
      let src =
        'https://player.vimeo.com/video/' + id + '?background=1&controls=0'
      if (this.params.muted) {
        src += '&muted=1'
      }
      if (
  ...
  var VideoBackgrounds = class {
    ...
    add(element, params) {
      ...
      const link =
        element.getAttribute('data-youtube') || element.getAttribute('data-vbg')
      ...
      switch (vid_data.type) {
        case 'YOUTUBE':
          const yb = new YoutubeBackground(
            element,
            params,
            vid_data.id,
            uid,
    ...
    get(element) {
      const uid =
        typeof element === 'string'
          ? element
          : element.getAttribute('data-vbg-uid')
