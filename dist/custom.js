jQuery(document).ready(function ($) {
  ...
  const $notRightNow = $(".not_right_now");
  const $rightCaptcha = $(".open_right_captcha");
  const $labelOpen = $(".not_right_now label input[type='checkbox']");
  const $btn2 = $("button.heating-cooling-button-2");
  const $btn3 = $("button.ev-button-2");
  const $btn4 = $("button.battery-storage-button-2"); // for all three btns make fn later

  $labelOpen.each(function (i, ele) {
    const $checkbox = $(this);
    if ($checkbox.val() === "Not right now") {
      if (
        $checkbox.is(":checked") ||
        $checkbox.parent("label").hasClass("open")
      ) {
        $rightCaptcha.addClass("show-captcha");
        $btn2.text("Submit");
        $btn3.text("Submit");
        $btn4.text("Submit");
      }
  ...
  $notRightNow.on("click", function (e) {
    if (e.target.value === "Not right now") {
      $rightCaptcha.addClass("show-captcha");
      $btn2.text("Submit");
      $btn3.text("Submit");
      $btn4.text("Submit");
    } else {
      $rightCaptcha.removeClass("show-captcha");
      $btn2.text("Next");
      $btn3.text("Next");
      $btn4.text("Next");
    }
  ...
  /*-- new requirement --*/
  var solarlink =
    "https://aecon.parachutedevelopment.ca/free-assessment-solar/";
  var evecharginglink =
  ...
  contactform.on("submit", function (event) {
    ...
    const checkValue = $("input[type='checkbox']#field_rcxd6-0").prop(
      "checked"
    );
